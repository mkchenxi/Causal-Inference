Tutorial: RDD for Keywords Bidding
================
Xi Chen
2026-06-22

- [Data Visualization](#data-visualization)
- [Selecting A Bandwidth with
  Cross-Validation](#selecting-a-bandwidth-with-cross-validation)
- [Estimating the Position Effects](#estimating-the-position-effects)
...
ggplot(keyword_bid, aes(adrank, revenue, color = adposition)) + 
  ...
  scale_color_manual(values = c("#3B4992FF", "#E18727FF"))
```
...
(the width of the bins) $`h`$ and for the number of bins $`K_0`$ to the
left of the cutoff $`c`$ and $`K_1`$ to the right. We can construct bins
with the running variable $`X_i \in (b_k, b_{k+1}]`$,
$`\forall k=1,\cdots,K_0+K_1`$, where:
``` math
b_k=c-(K_0-k+1)h.
```
The average value of the outcome variable $`Y_k`$:
``` math
